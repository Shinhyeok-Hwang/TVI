...
chrome.storage.local.get(['initialized'], function(result) {
    if(result.length = 0){
        chrome.storage.local.set({'initialized': 1}, function() {
            console.log("initialized");
        });
        chrome.storage.local.set({'activated': 1}, function(){
            console.log("activated");
        })
        chrome.storage.local.set({'thanos_power': 3}, function(){
            console.log("thanos power is set to 3");
        })
        chrome.storage.local.set({'thanos_vacation':30}, function(){
            console.log("thanos vacation is set to 30");
        })
        chrome.storage.local.set({'ironman_love':3000}, function(){
            console.log("ironman love is set to 3000");
        })
    }
});
